...
*	Student id : 	32143199	3213172		32121826
*	Student name : 	이민승		윤지원		서영준
* 	Group name : 	한조
*
*   lab1_sched.c :
*       - Lab1 source file.
...
void sorting(Job job[], char sortmap[], int size){
...
for (i=0;i<size;i++)
{
	sortmap[i]=job[i].name;
	job[i].name = 65+i;
}
}

void inverse_sorting(char Result[][7], char sortmap[], int total_length)
{
...
for (i=0;i<7;i++)
{
	...
	{	
	Result[j][i]=sortmap[Result[j][i]-65];
	}
}
}
	...
	void FIFO(int size, int length, Job job[], char Result[][7]){
		...
		int st[size]; // change to size
		for (i = 0; i < size; i++)
		{
			st[i] = job[i].service_time;
		}

		for (clock = job[0].arrival_time; clock < length; clock++)
		{
			for (i = 0; i < size; i++)
			{
				if (job[i].arrival_time == clock)
				{
					PushQueue(&Q_fifo, &job[i]);
				}
			}
			if (Q_fifo.first != NULL)
			{
				Result[clock][0] = Q_fifo.first->name;
				st[Q_fifo.first->name - 65]--;
				if (st[Q_fifo.first->name - 65] == 0) {
					tmp = *PopQueue(&Q_fifo);
				}
			}
		}
	...
	void Round_Robin(int size, int length, Job job[], char Result[][7], int time_slice, int version){
		...
		int inq[size]; // change to size
		for (i = 0; i < size; i++)
		{
			inq[i] = 0;
		}

		PushQueue(&Q_rr, &job[0]);
		inq[0] = 1;

		for (clock = job[0].arrival_time; clock < length;)
		{
			if (Q_rr.first == NULL)
			{
				clock++;
			...
			if (st[tmp.name - 65] <= time_slice) // smaller than time_slice
			{
				for (i = 0; i < st[tmp.name - 65]; i++)
				{
					Result[clock][version] = job[tmp.name - 65].name;
					clock++;//
				}
				st[tmp.name - 65] = 0;

				for (j = 0; j < size; j++) // tie-break rule : new job
				{
					if (job[j].arrival_time <= clock && st[j] != 0 && inq[j] != 1)
					{
						PushQueue(&Q_rr, &job[j]);
						inq[j] = 1;
					}
				}
			}
			else // bigger than time_slice
			{
				...
				for (j = 0; j < size; j++) // tie-break rule : new job
				{
					if (job[j].arrival_time <= clock && st[j] != 0 && (tmp.name - 65) != j && inq[j] != 1)
					{
						PushQueue(&Q_rr, &job[j]);
						inq[j] = 1;
					}
				...
				// PushQueue the uncompleted job
				PushQueue(&Q_rr, &job[tmp.name - 65]);
				inq[tmp.name - 65] = 1;
